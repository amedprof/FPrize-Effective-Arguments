...
from data.dataset import FeedbackDataset,CustomCollator,FeedbackDatasetInference,CustomCollatorInference
...
class AWP:
    def __init__(
        ...
        self.scaler = scaler
        self.args = args
        self.criterion = eval(args.model['loss'])(reduction="none").to(args.device)
...
def training_step(args,model,criterion,data):
    ...
    span_labels = data["span_labels"][mask]
    nb = span_preds.shape[0]
    loss = criterion(span_preds,span_labels[:nb])
...
class FeedbackDatasetInferenceVf(Dataset):
    def __init__(self,
                 output_folder,
                ):
        self.ids = [x for x in os.listdir(output_folder) if 'id_' in x]
        self.folder = output_folder
    def __len__(self) -> int:
        return len(self.ids)
    
    def __getitem__(self,idx):
        return load_pickle(self.folder+self.ids[idx])

# def load_checkpoint_data(args,checkpoint,size_tokenizer):
#     print(checkpoint)
...
#         net.backbone.resize_token_embeddings(size_tokenizer)

    
#     dummy_device = args.device #torch.device("cpu")
#     net = net.to(dummy_device)
#     d = torch.load(checkpoint, map_location=dummy_device)
#     try:
#         net.load_state_dict(d)
#     except:
...
def prediction_step(args,df_test,checkpoints,weights,output_folder="/kaggle/working/"):
    
    print("creating dataset")
    tokenizer = AutoTokenizer.from_pretrained(args.model['pretrained_tokenizer'])
    data_collator = CustomCollatorInference(tokenizer)

    test_dataset = FeedbackDatasetInference(df_test,tokenizer,args.model["max_len"],data_folder=args.data_folder_test,
                                    add_special_tokens=args.add_special_tokens,
                                    input_type=args.input_type,
                                    mask_pct=0
                                    )
    # for i,g in enumerate(data_dataset):
    #     save_pickle(output_folder+f"id_{i}",g)

    # df = data_dataset.df.copy()

    # del data_dataset
    # gc.collect()
    # print("delete dataset")

    # test_dataset = FeedbackDatasetInferenceVf(output_folder)
...
def train_one_fold(args,tokenizer,train_df,valid_df,fold):    
    ...
    val_dataset = FeedbackDataset(valid_df,tokenizer,args.model["max_len_eval"],
                                    data_folder=args.data_folder/'train',
                                    add_special_tokens=args.add_special_tokens,

...
class args:
    ...
    model = {
            ...
            "pretrained_tokenizer":Path(fr'../input/debertaxlargeacv1/tokenizer/tokenizer'),
            "pretrained_config":Path(fr'../input/debertaxlargeacv1/config.pth'),
            "max_len":2048,
...
checkpoints = [x.as_posix() for x in (Path(fr'../input/debertaxlargeacv1')).glob("*.pth") if 'config' not in str(x)]
...
sub[["discourse_id",'Ineffective','Adequate','Effective']].to_csv('deberta_xlarge_ac.csv',index=False)
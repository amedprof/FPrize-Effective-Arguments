...
class args:
    ...
    model = {
            ...
            "pretrained_tokenizer":Path(fr'../input/checkpoints/deberta_v3_large_tokenizer/deberta_v3_large_tokenizer/tokenizer/tokenizer'),
            "pretrained_config":Path(fr'../input/checkpoints/deberta_v3_large_tokenizer/deberta_v3_large_tokenizer/config.pth'),
            "max_len":2048,
...
checkpoints = [x.as_posix() for x in (Path(fr'../input/psl-deberta-v3-large')).glob("*.pth") if 'config' not in str(x)]
...
sub[["discourse_id",'Ineffective','Adequate','Effective']].to_csv('psl_deberta_v3_large_ac.csv',index=False)
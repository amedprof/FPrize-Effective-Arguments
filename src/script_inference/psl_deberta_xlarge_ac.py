...
class args:
    ...
    # Model
    model_name = "microsoft/deberta-xlarge" #"funnel-transformer/large" #"allenai/longformer-large-4096"
    ...
    model = {
            ...
            "pretrained_tokenizer":Path(fr'../input/checkpoints/deberta_xlarge_tokenizer/deberta_xlarge_tokenizer/tokenizer/tokenizer'),
            "pretrained_config":Path(fr'../input/checkpoints/deberta_xlarge_tokenizer/deberta_xlarge_tokenizer/config.pth'),
            "max_len":2048,
...
checkpoints = [x.as_posix() for x in (Path(fr'../input/deberta-xlarge-all-data')).glob("*.pth") if 'config' not in str(x)]
...
sub[["discourse_id",'Ineffective','Adequate','Effective']].to_csv('psl_deberta_xlarge_ac.csv',index=False)